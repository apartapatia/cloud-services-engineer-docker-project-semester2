...
Добавлен дроп всех capabilites, по принципу минимальной доступности.
...
В проекте настроено автоматическое сканирование на наличие уязвимостей в CI/CD. Для этого используется сканер **Trivy**. При каждом push и pull request собираются локальные образы фронтенда и бекенда, после чего Trivy сканирует их на уязвимости ОС и библиотек уровня `CRITICAL` и `HIGH`. Результат пайплайна отображается в виде бейджа в начале этого документа.
